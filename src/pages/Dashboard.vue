      ...
      <header class="bg-blue-700 text-white shadow p-4 flex justify-between items-center relative">
        ...
        <div class="relative">
          <button @click="toggleMenu" class="flex items-center focus:outline-none">
            <img src="https://i.pravatar.cc/40" alt="Avatar" class="w-10 h-10 rounded-full border-2 border-white"/>
          ...
          <div v-show="menuOpen" class="absolute right-0 mt-2 w-48 bg-white rounded shadow-lg z-20">
            <button @click="handleLogout" class="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100">Logout</button>
          </div>
        </div>
      </header>
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody>
                <tr v-for="arrival in arrivals" :key="arrival.id" class="hover:bg-gray-50">
                  ...
                  <td class="p-3 border-b font-semibold" :class="statusColor(arrival.status)">{{ arrival.status }}</td>
                  <td class="p-3 border-b">
                    <button class="text-blue-600 hover:underline" @click="openModal(arrival)">
                      Detail
                    </button>
                  </td>
                </tr>
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center">
      <div class="bg-white rounded-lg shadow-lg w-full max-w-lg p-6 relative">
        ...
        <div class="mt-6 flex space-x-4">
          <button class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded" @click="approveArrival">Approve</button>
          <button class="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 rounded" @click="rejectArrival">Reject</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async approveArrival() {
      try {
        await api.post(`/arrivals/${this.selectedArrival.id}/approve`);
    ...
    async rejectArrival() {
      try {
        await api.post(`/arrivals/${this.selectedArrival.id}/reject`);
        await this.fetchArrivals();
      } catch (err) {
